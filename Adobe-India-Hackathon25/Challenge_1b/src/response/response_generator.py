...
# Try to import Llamafile, but handle import errors gracefully
try:
...
class ResponseGenerator:
    ...
    def __init__(self, config: Dict):
        ...
        # Initialize LLM lazily to avoid connection issues during startup
        self.llm = None
    ...
    def _initialize_llm(self):
        ...
        if not self._llm_initialized:
            if not LLAMAFILE_AVAILABLE:
                self.logger.warning("Llamafile not available, using mock response generator")
                self.llm = None
                ...
                try:
                    ...
                    self.logger.warning(f"LLM server not available, using mock response generator: {e}")
                    self.llm = None
                    self._llm_initialized = True
    
    def generate_refined_text(self, content: str, title: str) -> str:
        """
        Generate refined text analysis for a section
        ...
        Returns:
            str: Refined text analysis
        """
        self._initialize_llm()
        
        if self.llm is None:
            # Mock refined text when LLM is not available
            return f"Refined analysis of '{title}': {content[:200]}..."
        
        prompt = ChatPromptTemplate.from_template("""
        Please provide a refined analysis of the following content section.
        
        Section Title: {title}
        Content: {content}
        
        Provide a concise, well-structured analysis that:
        1. Summarizes the key points
        2. Highlights important details
        3. Maintains clarity and readability
        4. Is relevant to the section title
        
        Refined Analysis:
        """)
        
        messages = prompt.format_messages(title=title, content=content)
        response = self.llm.invoke(messages)
    ...
    def expand_query(self, query: str) -> str:
        ...
        if self.llm is None:
            # Return original query when LLM is not available
            return query
