...
from .file_converter_windows import OfficeConverterWindows
...
class FileLoader(BaseLoader):
    ...
    def __init__(self, config):
        ...
        # Use Windows-compatible converter if on Windows
        import platform
        if platform.system().lower() == "windows":
            try:
                self.file_converter = OfficeConverterWindows()
            except Exception as e:
                self.logger.warning(f"Failed to initialize Windows converter: {e}")
                self.file_converter = None
        else:
            try:
                self.file_converter = OfficeConverter()
            except Exception as e:
                self.logger.warning(f"Failed to initialize LibreOffice converter: {e}")
                self.file_converter = None
